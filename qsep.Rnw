...
diseases~\citep{Shin:2013,Cody:2013}.
...
\citep{Gatto:2014} have been documented, that confidently infer the
localisation of thousands of proteins. have been documented. Most
techniques rely on some form of sub-cellular fractionation, many
employing differential centrifugation or separation along density
gradients, and the subsequent quantitative assessment of relative
protein occupancy profiles in these sub-cellular
fractions. Reciprocally, a broad array of computational methods have
been applied, ranging from unsupervised learning e.g.clustering,
dimensionality reduction \citep{Tomizioli:2014}, supervised learning
e.g. classification (reviewed in \citep{Gatto:2014}, semi-supervised
learning and novelty detection \citep{Breckels:2013} and, more
recently, transfer learning \citep{Breckels:2016}.
...
niches (also referred to as protein clusters, or only clusters) that
...
or \citet{Nikolovski:2014} (\textit{Label free protein quantification
  for plant Golgi protein localisation and abundance}) have a much
...
BB/L002817/1). LMB is supported by a Wellcome Trust Technology
Development Grant (Grant number 108441/Z/15/Z). KSL is a Wellcome
Trust Joint Investigator (110170/Z/15/Z). The authors would like to
