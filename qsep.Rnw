...
methods have been applied, ranging from unsupervised learning e.g.clustering
\citep{Tomizioli:2014}, supervised learning e.g. classification (reviewed in
\citep{Gatto:2014}, semi-supervised learning and novelty detection \citep{Breckels:2013}
...
repository\footnote{\url{https://github.com/ComputationalProteomicsUnit/QSep-manuscript}}.
...
pdtab$Title <- 
    paste(pdtab$Title, c("\\citep{Christoforou:2016}", "\\citep{Christoforou:2016}",
                         "\\citep{Breckels:2013}", "\\citep{Itzhak:2016}",
                         "\\citep{RodriguezPineiro:2012}", "\\citep{Tan:2009}",
                         "\\citep{Breckels:2016}", "\\citep{Trotter:2010}",
                         "\\citep{Dunkley:2006}", "\\citep{Foster:2006}",
                         "\\citep{Nikolovski:2014}", "\\citep{Groen:2014}",
                         "\\citep{Nikolovski:2012}", "\\citep{Andreyev:2010}",
                         "\\citep{Hall:2009}"))
@
...
(\textit{Identification of trans-golgi network proteins in Arabidopsis
  ...
  is targetted at resolving a particular niche, it
  is often the case that other sub-compartments are less well-resolved as 
  a sacrafice and hence, it is important to keep the overall
...
(\Biocpkg{pRoloc}'s \texttt{plotDist} function can be used for that)
...
in this meta-analysis is out of scope.
...
the comparison of the average euclidean distance within and between
sub-cellular clusters. As illustrated on the heatmaps in
...
\textbf{QST:} I am not sure about this section. Does it not feel like
I am having a go at \citet{Itzhak:2016}?
...
refer back to always refer the original data when considering
summerising metrics like these, to put the resolution into context;
...
quantitative assement boxplots and heatmaps are shown in
...
proven quite successfull \citep{Breckels:2013} and has identified
previously undetected sub-cellular niches that form tight and
well-resolved clusters (see for example ribosomial and trans-golgi
network (TGN) in \textit{dunkley2006}, or proteasome and nucleus in
\textit{hall2009r1} to cite only a few), which in turn favour good
resolution scores.


The \textit{hall2009} dataset is relatively poorly annotated (only 5
sub-cellular clusters, which is the lowest in out test datasets). As
long as these few clusters are well separated, poor annotation can
positively influences the resolution scoring. Hence, it is important
to visualise the data as described in section~\ref{sec:vis} and
consider the sub-cellular diversity exposed in the analysed data (see
...
\textit{nikolovski2012imp}. Note that the quantiative separation
...
authors in the \textit{itzhak2016stcSILAC} data could be dropped on
base on its loose definition and lack of resolution. This would only
...
out of \Sexpr{nitz^2} distances) and would not change the position of
the dataset.

As mentioned earlier, the \textit{groen2014cmb} and
\textit{nikolovski2014} are tagetted experiment, focusing on the
trans-Golgi and Golgi niches respectively. Such experiments do not aim
for best global resolution, which is reflected by relatively low
resolution.

The \textit{foster2006} experiment display relatively poor
...
\begin{figure}[p]
  ...
  \caption{Quantiative separation boxplot for the \Sexpr{nrow(pdata)}
...
\begin{figure}[p]
  ...
  \caption{Quantiative separation heatmaps for the \Sexpr{nrow(pdata)}
    experiments used in this study. The experiments are ordered
    according to the median average between cluster distance (see
    figure~\ref{fig:qsep}). }
  \label{fig:allhmaps}
...
(\textbf{TODO}: Say someething about number and tighness of
...
1.13.9 and 1.11.6 or later, respectively.
