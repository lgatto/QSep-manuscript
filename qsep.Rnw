...
(reviewed in \citep{Gatto:2010}) and computational methods
...
of sub-cellular fractionation using differential centrifugation or
...
methods have been applied, ranging from unsupervised learning
e.g.clustering, dimensionality reduction \citep{Tomizioli:2014},
supervised learning e.g. classification (reviewed in
...
\citep{Gatto:2014a}. In the interest of readability, we do not display
all details in this rendering of the manuscript. Nevertheless, the
code to reproduce all results and figures is available in the source
of this document in the public manuscript
repository~\cite{qseprepo}.

\section{Spatial proteomics datasets}\label{sec:pdata}

<<pdata>>=
.experiments <- c("E14TG2aS1", "andy2011", "andreyev2009rest", "dunkley2006",
...
## This is added manually and depends on the order set in the previous line
...
year <- c(2011, ## E14 from Breckels 2016
          2010, ## Andreyev
          2011, ## andy2011, from Breckels 2013
...
limited number of markers was provided by the authors\footnote{This
  ...
  localisation prediction}. We have also only considered clusters that
were defined by at least 7 markers\footnote{This number is relatively
  low, and we would typically recommend at least 13 markers per class
  to perform cross-validation when optimising classifier
  parameters. See \citet{Gatto:2014} and the main \Biocpkg{pRoloc}
  tutorial for details.}. Where a combined dataset of multiple replicated 
  experiments are provided by the original authors, we have used the
  combined dataset, rather than individual replicates, as this generally
  refers to the dataset with best sub-cellular
resolution \citep{Trotter:2010}. If there is no combined dataset, or only a
single replicate is reported we 
have taken the (only or) first replicate experiment. 
In addition, for dimensionality
reduction and visualisation, we have systematically replaced missing
values by zeros. When calculating distances between protein profiles
(see section~\ref{sec:qsep}), however, missing values were retained.
...
(\textit{Identification of trans-golgi network proteins in Arabidopsis
  ...
  abundance}) have a much more targeted goal (identification of trans-Golgi
  -network and Golgi apparatus proteins, respectively). When an experiment
  is targeted at resolving a particular niche, it
  is often the case that other sub-compartments are less well-resolved as 
  a sacrifice and hence, it is important to keep the overall
...
compare the full occupancy profiles of the marker proteins
(\Biocpkg{pRoloc}'s \texttt{plotDist} function can be used for this)
or visualise a dendrogram representing the average distance between
cluster profiles (the \texttt{mrkHClust} function from
\Biocpkg{pRoloc} offers this functionality). While detailed
exploration of a dataset using these and other visualisation (we also
recommend the interactive visualisation apps from the
\Biocpkg{pRolocGUI} package \citet{pRolocGUI}) is crucial before
analysing and interpreting a new spatial proteomics experiment, a
detailed exploration of each of the \Sexpr{nrow(pdtab)} datasets used
in this meta-analysis is out of the scope of this analysis.
...
symmetrical (i.e. the distance bee tween cluster 1 and 2 is the same as
...
between experiments (see section~\ref{sec:compara} for the latter),
we further divide each distance by the reference within cluster
...
symmetrical anymore. To facilitate the comparison of distances between
...
(as well as \textit{andy2011}, discussed later) have either been
...
This re-analysis has proven quite successful \citep{Breckels:2013} and
has identified previously undetected sub-cellular niches that form
tight and well-resolved clusters (see for example ribosomial and
trans-golgi network (TGN) in \textit{dunkley2006}, or proteasome and
nucleus in \textit{tan2009r1} to cite only a few), which in turn
favour good resolution scores. The \textit{hall2009} dataset is
relatively poorly annotated (only 5 sub-cellular clusters, which is
the lowest in all test datasets). As long as these few clusters are
well separated, poor annotation will not negatively influence the
resolution scoring. Hence, it is important to visualise the data as
described in section~\ref{sec:vis} and consider the sub-cellular
diversity exposed in the analysed data (see
section~\ref{sec:sub-cell-divers}).
...
are \textit{andy2011}, \textit{itzhak2016stcSILAC} and
...
loosely defines many niches and thus lacks resolution. This would only
marginally influence the overall assessment metrics as only the
...
\textit{nikolovski2014} are targeted experiment, focusing on the
trans-Golgi-network and Golgi niches respectively. Such experiments do
not aim for best global resolution, which is reflected by relatively
low resolution.
...
values (\Sexpr{100*round(sum(is.na(foster2006))/prod(dim(foster2006)),
  3)} \%). Finally, the \textit{andreyev2009rest} dataset suffers from
...
In this section, we assess the resolution metric itself, and how the
...
The number of classes does not have any effect the resolution
...
It is necessary to emphasise the importance and effect of marker
definition on estimating and assessing the resolution of spatial
proteomics experiments and, of course, the assignment of proteins to
their most likely sub-cellular compartments. 


Is there an improvement in resolution with time?

\begin{figure}[h]
  \centering
<<restime>>=

## re-analysed in Breckels:2013:
lopit <- c("hyperLOPIT2015", "hyperLOPIT2015ms2", "andy2011", 
           "tan2009r1", "E14TG2aS1", "trotter2010", "dunkley2006",
           "groen2014cmb", "nikolovski2012imp", "hall2009")

.pos <- rep(3, length(pdataitems))
.pos[pdataitems == "trotter2010"] <- 2
.pos[pdataitems == "hall2009"] <- 1
.pos[grep("hyperLOPIT", pdataitems)] <- 1

plot(mds ~ pdtab$year, xlim = c(2005, 2017),
     xlab = "Original year of publication",
     ylab = "Quantitative assessment (medians)")
grid()
text(pdtab$year, mds, pdataitems, cex = .8, 
     pos = .pos)
@  
\caption{Resolution of spatial proteomics experiments over time. The
  assigned dates matches either the original publication or, when
  know, the actual date the data was generated. For publications that
...
taken as absolute. Its main purpose is to provide a guide to compare
different experiments. It will be useful for laboratories that do
spatial studies on different models and with different fractionation
and/or quantitation methods, to assess the impact of these variables
(such as, for example hyperLOPIT MS$^2$ and MS$^3$). It is also useful
to compare separation between different labs, as demonstrated in our
...
Sub-cellular resolution is of course only one aspect of spatial
proteomics, albeit an essential one, that critically determines the
reliability of protein assignments to spatial niches as well as the
identification of multi- and trans-localisation events.
...
1.13.15 and 1.11.7 or later, respectively. The source of this document
...
Longer and Larger grant (Award BB/L002817/1). The authors would like
to thank Dr Claire M. Mulvey for helpful comments on the quantitative
assessment.
