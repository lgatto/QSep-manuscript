...
\textit{\Sexpr{pdtab[which.max(pdtab$Clusters), 1]}}). These numbers
should be assessed in the light of about 25 different sub-cellular
niches that are documented in all \Sexpr{nrow(pdtab)} datasets, which
...
of removed classes, the slopes are \Sexpr{round(e14slope, 3)} and
...
t-test produces a p-value of \Sexpr{signif(p1, 2)}). Annotating
...
metric (p-value of \Sexpr{signif(p2, 2)}, log-transformed QSep
...
limited number of datasets (\Sexpr{nrow(pdtab)} dataset, out of
\Sexpr{npubs} individual publications, some including data
