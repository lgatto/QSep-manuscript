...
This re-analysis has proven quite successfull \citep{Breckels:2013}
and has identified previously undetected sub-cellular niches that form
...
nucleus in \textit{hall2009r1} to cite only a few), which in turn
favour good resolution scores.
...
positively influence the resolution scoring. Hence, it is important
...
(\textbf{TODO}: Say something about number and tightness of
