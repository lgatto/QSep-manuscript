...
were defined by at least 7 markers\footnote{This number is relatively
  ...
  tutorial for details.}. When provided by the original authors, we
have combined multiple replicated experiments to improve sub-cellular
resolution \citep{Trotter:2010}. In addition, for dimensionality
