...
rediction technique. It project the protein occupancy profiles into a
new space in such a way as to maximise the spread of all points
(i.e. labelled and unlabelled proteins) along the first new dimension
(principal component, PC). The second PC is then chosen to be
perpendicular to the first one while maximising the overall
variability. Each PC (there are as many as there are fractions)
accounts for a percentage of the total variability and it is not
uncommon, in well exectuted experiments, that the two first PCs
...
Position of clusters on PCA plot. Comparing PCA and LDA.
