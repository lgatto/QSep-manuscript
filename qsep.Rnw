...
\caption{Resolution of spatial proteomics experiments over time. The
  ...
  most recent one was used. Experiments are colour-coloded based on
  whether they used label-free (LF), SILAC or isobaric tagging (TMT or
...
It is very difficult to draw conclusion whether any quantitation
technologies, instruments or species correlate with better resolution
due to the limited number of datasets (\Sexpr{nrow(pdtab)} dataset,
out of \Sexpr{npubs} individual publications), as well as confounding
factors such as the laboratory the data originates from and time. As
an example of the latter, the distinction between iTRAQ and TMT
tagging would falsly suggest that the latter (used in the
better-performing \texttt{beltran} and \texttt{hyperLOPIT} expriments)
are better, while this trend is rather a result of the field moving
towards systems with more tags.
