...
figure~\ref{fig:mrkswtch-pca}, left) and calculated the quantitative
resolution metrics (see boxplots on figure~\ref{fig:mrkswtch-pca},
right). 
...
their most likely sub-cellular compartments. In this work, we have
used the markers provided by the original authors (except for
...
1.13.14 and 1.11.7 or later, respectively. The source of this document
enabling to reproduce all results and figures is available in the
source of this document in the public manuscript repository~\cite{qseprepo}.
